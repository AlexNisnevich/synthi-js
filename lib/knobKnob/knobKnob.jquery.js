...
(function($){
	...
	$.fn.knobKnob = function(props){
		...
		return this.each(function(){
			...
			knob.mouseenter(function() { mouseOverKnob = true;})
				.mouseleave(function() { mouseOverKnob = false;});
			...
			knob.on('mousedown touchstart', function(e){
				...
				offset.bottom = offset.top + knob.height();
				...
				$(document).on('mousemove.rem touchmove.rem',function(e){
					...
					if (mouseOverKnob) {
						...
						if (e.pageY < offset.top) {
							var rawDelta = (offset.top - e.pageY) / $(document).height() * 100;
						} else if (e.pageY > offset.bottom) {
							var rawDelta = (offset.bottom - e.pageY) / $(document).height() * 100;
						}

						var multiplier = Math.max(Math.log(Math.abs(rawDelta)) / 2, 1);
						var delta = rawDelta * multiplier;
						console.log(rawDelta + " => " + delta);
