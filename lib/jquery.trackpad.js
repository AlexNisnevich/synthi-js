    ...
    return this.each(function() {
      ...
      el.on("mousedown", function(ev) {
        ...
        function moveTo(ev) {
          ...
          if (topOffset < 0 || topOffset > height || leftOffset < 0 || leftOffset > width) {
            var transform = "none";
          } else {
            var transform = "translate(" + (leftOffset - width/2) + "px," + (topOffset - height/2) + "px)";
          }

          clearTimeout(drawQueue);
          drawQueue = setTimeout(function () {
            marker.css('transform', transform);
          }, 5);
        }
